...
$recrute_audio_url = function_exists( 'tpmeta_field' ) ? tpmeta_field( 'recrute_post_audio' ) : NULL;
...
?>
 ...
 <article id="post-<?php the_ID();?>" <?php post_class( 'postbox__item vl-postbox-item-wrapper mb-80 format-image' );?>>
     ...
     <?php echo wp_oembed_get( $recrute_audio_url ); ?>
     </div>
     <?php endif; ?>
     <!-- blog meta -->
...
<article id="post-<?php the_ID();?>" <?php post_class( 'vl-postbox-item mb-50 format-audio' );?>>
    <?php if ( has_post_thumbnail() ): ?>
