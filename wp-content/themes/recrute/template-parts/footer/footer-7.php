...
$footer_cta_image_img = !empty($recrute_footer_cta_image_url_from_page['url']) ? $recrute_footer_cta_image_url_from_page['url'] : $cta_image;
  ...
  if ( is_active_sidebar('footer-7-1') OR is_active_sidebar('footer-7-2') OR is_active_sidebar('footer-7-3') OR is_active_sidebar('footer-7-4') ){
...
<!--=====CTA AREA START=======-->
<div class="cta7"
     style="background-image:url(<?php echo esc_url($footer_cta_bg_img ); ?>); background-color:<?php echo esc_attr($footer_cta_bg_color); ?>">
     <div class="container">
...
<!--===== FOOTER AREA START =======-->
<div class="footer7 _relative <?php echo esc_attr($footertopSpecing); ?>" style="background-image:url(<?php echo esc_url($bg_img ); ?>); background-color:<?php echo esc_attr($bg_color); ?>">
<?php if ( is_active_sidebar('footer-7-1') OR is_active_sidebar('footer-7-2') OR is_active_sidebar('footer-7-3') OR is_active_sidebar('footer-7-4') ): ?>
     <div class="container">
          <div class="row">
          <?php
                    if ( $footer_columns < 5 ) {
                        ...
                        dynamic_sidebar( 'footer-7-4' );
                        print '</div>';
                    } else {
                    for ( $num = 1; $num <= $footer_columns; $num++ ) {
                    if ( !is_active_sidebar( 'footer-7-' . $num ) ) {
                    continue;
                    }
                        print '<div class="' . esc_attr( $footer_class[$num] ) . '">';
                        dynamic_sidebar( 'footer-7-' . $num );
                        print '</div>';
                    }
                    }
                ?>
